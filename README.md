...
- `GITHUB_USERNAME` — username to scrape
- `GITHUB_PDF_FOLDER` — folder to save generated PDFs
- `METADATA_JSON_PATH` — metadata file location
- `VECTORDB_PATH` — path for persistent ChromaDB storage
- `EMBEDDING_MODEL_NAME` — embedding model (default `all-MiniLM-L6-v2`)
- `TOP_K_MATCHES` — number of RAG results to include
- `GPT_MODEL_NAME` — model used by the assistant (default `gpt-5-mini`)

Environment variables:

- `TOKEN_GITHUB` — **required** GitHub personal access token for scraping private/read limits.
...
- `rag_assisted_chatbot/` — package modules
  - `github_scrapper.py` — GitHub README → PDF
  - `build_vectordb.py` — builds & persists ChromaDB embeddings
  - `ask_vectordb.py` — queries vector DB
  - `main.py` — `Assistant` class and CLI-style loop
  - `prompts.py`, `references.py` — prompts and static resume content
  - `output_structure.py` — pydantic models for structured outputs
  - `config.py` — defaults you can edit
- `scrapped_data/`, `scrapped_metadata/` — output folders used by scrapper
- `vectordb_storage/` — persistent ChromaDB files
